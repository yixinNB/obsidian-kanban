...
export function getDateColorFn(stateManager: StateManager): (date: moment.Moment) => DateColorKey | null {
   const dateColors = stateManager.getSetting('date-colors');
   const orders = (dateColors || []).map<[moment.Moment | 'today' | 'before' | 'after', DateColorKey]>(
      (c) => {
         if (c.isToday) return ['today', c];
         if (c.isBefore) return ['before', c];
         if (c.isAfter) return ['after', c];
