...
export default class KanbanPlugin extends Plugin {
   ...
   /**
    * 在mount()中注册,保存注册过的ob窗口
    * @ {window:{viewmap,viewStateReceivers,approot:htmlelement}}
    */
   ...
   /**
    * window->this.getKanbanViews返回的state
    * @param win
    */
   ...
   handleViewFileRename(view: KanbanView, oldPath: string) {
      ...
      if (reg.viewMap.has(oldId)) reg.viewMap.delete(oldId);
      if (!reg.viewMap.has(view.id)) reg.viewMap.set(view.id, view);
      if (view.isPrimary) this.getStateManager(view.file).softRefresh();
   }
   ...
   unmount(win: Window) {
      ...
      for (const view of reg.viewMap.values()) view.destroy();
